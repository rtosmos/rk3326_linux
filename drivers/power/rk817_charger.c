...
printk("rk817_charge_dc_det_isr\n");
	...
	a = gpio_get_value(charge->pdata->dc_det_pin);
printk("gpio:%d\n",a);
#if defined(CONFIG_ARCH_ROCKCHIP_ODROIDGOA)
...
static enum charger_t rk817_charge_get_dc_state(struct rk817_charger *charge)
{
	int level;
	int b;
printk("charger_t rk817_charge_get_dc_state\n");
b=gpio_get_value(charge->pdata->dc_det_pin);
printk("rk817_charge_get_dc_state gpio:%d\n",b);

	//if (!gpio_is_valid(charge->pdata->dc_det_pin))
	if (!gpio_is_valid(charge->pdata->dc_det_pin))
		{printk("charger_t rk817_charge_get_dc_state DC_TYPE_NONE_CHARGER\n");return DC_TYPE_NONE_CHARGER;}
